...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Spider~2.0}~\cite{yu2024spider2}: 152 databases used across 547 tasks in the Snowflake-only slice (\texttt{spider2-snow}), hosted on a sponsored Snowflake instance. Schemas are introspected via \texttt{INFORMATION\_SCHEMA}. Only the 152 task-relevant databases are ingested (filtered against the \texttt{spider2-snow.jsonl} task list).
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Spider~2.0 Snowflake bootstrap}: connects to the Snowflake instance via TOTP-authenticated programmatic access and queries \texttt{INFORMATION\_SCHEMA} per database. The script filters to only the 152 task-relevant databases (from \texttt{spider2-snow.jsonl}) and is designed for resumability---already-ingested databases are detected at startup and skipped.
\end{enumerate}
...
Spider~2.0 bootstrap     & Complete     & 123/152 task databases ingested; 29 remaining \\
...
We downloaded and analyzed all 69 documents from the \texttt{spider2-snow} benchmark (547 tasks total), finding that a significant fraction explicitly reference at least one knowledge document (exact count to be updated after rerunning analysis against \texttt{spider2-snow}).
...
Upon inspecting the Spider~2.0 task dataset, we found only 152 databases are referenced by the \texttt{spider2-snow} Snowflake benchmark tasks (547 tasks total), compared to 114 in the lite subset.
The bootstrap filters against the \texttt{spider2-snow.jsonl} task list, reducing unnecessary ingestion of irrelevant public datasets.
We also switched from \texttt{spider2-lite} (260 tasks, 114 DBs) to the full Snowflake slice \texttt{spider2-snow} (547 tasks, 152 DBs) to improve per-database task density for our QA exemplar approach.
